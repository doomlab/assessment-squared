...
{}{\PatchFailed}
\makeatother
\keywords{measurement invariance, assessment, registered report}
...
\urlstyle{same}
\hypersetup{
  pdftitle={The Landscape of Measurement Invariance: A Database and Effect Size Analysis of Assessment},
  pdfauthor={Erin M. Buchanan1, Jeffrey M Pavlacic2, \& Savannah C. Lewis3},
  pdflang={en-EN},
  pdfkeywords={measurement invariance, assessment, registered report},
  hidelinks,
  pdfcreator={LaTeX via pandoc}}

\title{The Landscape of Measurement Invariance: A Database and Effect Size Analysis of Assessment}
\author{Erin M. Buchanan\textsuperscript{1}, Jeffrey M Pavlacic\textsuperscript{2}, \& Savannah C. Lewis\textsuperscript{3}}
\date{}
...
The authors made the following contributions. Erin M. Buchanan: Conceptualization, Project Administration, Writing - Original Draft Preparation; Jeffrey M Pavlacic: Conceptualization, Project Administration, Writing - Original Draft Preparation; Savannah C. Lewis: Project Administration, Writing - Original Draft Preparation.
...
\affiliation{\vspace{0.5cm}\textsuperscript{1} Harrisburg University of Science and Technology\\\textsuperscript{2} Medical University of South Carolina\\\textsuperscript{3} University of Alabama}

\abstract{%
Measurement invariance testing is essential for determining whether psychological instruments assess the same latent constructs across groups. Without invariance, observed group differences may reflect measurement artifacts rather than meaningful differences in the constructs being studied. Despite its importance, prior meta-research suggests that measurement invariance tests are infrequently reported and often difficult to reproduce. Furthermore, current practice typically relies on binary decision rules based on changes in fit indices, which provide limited information about the magnitude or practical importance of potential non-invariance. In this registered report, we propose a systematic investigation of measurement invariance analyses published in Assessment. We will compile a database of articles reporting measurement invariance tests and code key characteristics, including the instruments studied, groups compared, analytic procedures, and reported conclusions. For studies with accessible data, we will reproduce the reported analyses and estimate effect sizes for measurement invariance using the visualizemi R package. This approach uses bootstrapping and randomized group assignments to estimate the likelihood that observed invariance results would replicate in new samples. We will summarize the distributions of model-level and parameter-level effect sizes across studies. The resulting database and analyses will provide a resource for meta-research and offer empirical guidance for interpreting measurement invariance and defining smallest effect sizes of interest.
}
...
\begin{figure}
\includegraphics[width=1.69in]{../pics/data_curation} \caption{A flow chart of potential exclusions to create the database of measurement invariance (MI).}\label{fig:figure-process}
\end{figure}
...
\caption{\label{tab:table-1-example}Example output of `visualizemi` for model effect sizes}

\begin{tabular}{lllllll}
\toprule
model & \multicolumn{1}{c}{non\_invariant} & \multicolumn{1}{c}{random\_non\_invariant} & \multicolumn{1}{c}{h\_nmi} & \multicolumn{1}{c}{h\_mi} & \multicolumn{1}{c}{h\_nmi\_p} & \multicolumn{1}{c}{h\_mi\_p}\\
\midrule
intercepts & 0.15 & 0.01 & 0.59 & -0.59 & 0.19 & -0.19\\
loadings & 0.80 & 0.06 & 1.74 & -1.74 & 0.55 & -0.55\\
residuals & 0.04 & 0.06 & -0.11 & 0.11 & -0.03 & 0.03\\
\bottomrule
\addlinespace
\end{tabular}
...
If a model showed any partial invariance, we will then use the partial bootstrapping function in \texttt{visualizemi} to calculate the effect size of replicating partial invariance and the difference between parameter estimates. Given the \texttt{HolzingerSwineford1939} data shown in Table \ref{tab:table-1-example}, we would examine the results of partial invariance at the loadings step to determine where the potential group differences lie. Each factor loading would be individually relaxed one at a time and the bootstrapping procedure is examined for each loading separately. Table \ref{tab:table-2-example} demonstrates the results of the bootstrapping for the \texttt{HolzingerSwineford1939} data, including the number of non-invariant results for each individual factor loading. We will report \(h_{mni_p}\) for each parameter, along with \(d_s\) for the group size differences between groups for bootstrapped estimates. The effect size difference between parameter estimates is calculated by using \(d_s\) for independent samples groups. These estimates are calculated on the standardized loadings so they are comparable across different scale examples.
...
\caption{\label{tab:table-2-example}Example output of `visualizemi` for parameter effect sizes}

\begin{tabular}{lllllll}
\toprule
term & \multicolumn{1}{c}{non\_invariant} & \multicolumn{1}{c}{random\_non\_invariant} & \multicolumn{1}{c}{h\_nmi} & \multicolumn{1}{c}{h\_mi} & \multicolumn{1}{c}{h\_nmi\_p} & \multicolumn{1}{c}{h\_mi\_p}\\
\midrule
speed =\textasciitilde{} x7 & 0.96 & 0.80 & 0.50 & -0.50 & 0.16 & -0.16\\
speed =\textasciitilde{} x8 & 0.86 & 0.72 & 0.35 & -0.35 & 0.11 & -0.11\\
speed =\textasciitilde{} x9 & 0.88 & 0.76 & 0.31 & -0.31 & 0.10 & -0.10\\
textual =\textasciitilde{} x4 & 0.96 & 0.80 & 0.50 & -0.50 & 0.16 & -0.16\\
textual =\textasciitilde{} x5 & 0.96 & 0.80 & 0.51 & -0.51 & 0.16 & -0.16\\
textual =\textasciitilde{} x6 & 0.96 & 0.80 & 0.51 & -0.51 & 0.16 & -0.16\\
visual =\textasciitilde{} x1 & 0.96 & 0.80 & 0.50 & -0.50 & 0.16 & -0.16\\
visual =\textasciitilde{} x2 & 0.92 & 0.77 & 0.43 & -0.43 & 0.14 & -0.14\\
visual =\textasciitilde{} x3 & 0.92 & 0.78 & 0.41 & -0.41 & 0.13 & -0.13\\
\bottomrule
\addlinespace
\end{tabular}
...
Millsap, R. E. (2011). \emph{Statistical approaches to measurement invariance}. Routledge.
...
\bibitem[\citeproctext]{ref-rosseel2022}
Rosseel, Y., Jorgensen, T. D., Rockwood, N., Oberski, D., Byrnes, J., Vanbrabant, L., Savalei, V., Merkle, E., Hallquist, M., Rhemtulla, M., Katsikatsou, M., Barendse, M., Scharf, F., \& Du, H. (2022). \emph{Lavaan: Latent variable analysis}. \url{https://CRAN.R-project.org/package=lavaan}
...
\FunctionTok{library}\NormalTok{(visualizemi)}
\FunctionTok{library}\NormalTok{(rio)}
\CommentTok{\# set the seed}
\FunctionTok{set.seed}\NormalTok{(}\DecValTok{43843}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# import the dataset here }
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
                    \CommentTok{\# cfa model}
\NormalTok{saved\_mgcfa }\OtherTok{\textless{}{-}} \FunctionTok{mgcfa}\NormalTok{(}\AttributeTok{model =}\NormalTok{ HS.model,}
                     \CommentTok{\# dataset in data frame }
                     \AttributeTok{data =}\NormalTok{ HolzingerSwineford1939, }
                     \CommentTok{\# grouping variable column as a character}
                     \AttributeTok{group =} \StringTok{"sex"}\NormalTok{,}
                     \CommentTok{\# lavaan syntax for group constraints}
                     ...
                     \CommentTok{\# any other lavaan cfa arguments}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# review the model outputs to ensure they do not produce errors}
\NormalTok{saved\_mgcfa}\SpecialCharTok{$}\NormalTok{model\_fit}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\CommentTok{\# overall}
\NormalTok{saved\_mgcfa}\SpecialCharTok{$}\NormalTok{model\_overall}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\CommentTok{\# groups}
\CommentTok{\# saved\_mgcfa$group\_models$model.1}
\CommentTok{\# saved\_mgcfa$group\_models$model.2}

\CommentTok{\# configural}
\CommentTok{\# saved\_mgcfa$model\_configural}

\CommentTok{\# constraints}
\CommentTok{\# saved\_mgcfa$invariance\_models$model.loadings}
\CommentTok{\# saved\_mgcfa$invariance\_models$model.intercepts}
\CommentTok{\# saved\_mgcfa$invariance\_models$model.residuals}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{saved\_boot\_model }\OtherTok{\textless{}{-}} \FunctionTok{bootstrap\_model}\NormalTok{(}
  \CommentTok{\# saved configural model to start at}
  \AttributeTok{saved\_configural =}\NormalTok{ saved\_mgcfa}\SpecialCharTok{$}\NormalTok{model\_configural,}
  \CommentTok{\# dataset for the analysis}
  \AttributeTok{data =}\NormalTok{ HolzingerSwineford1939,}
  \CommentTok{\# model lavaan syntax}
  \AttributeTok{model =}\NormalTok{ HS.model,}
  \CommentTok{\# group variable in the dataset}
  \AttributeTok{group =} \StringTok{"sex"}\NormalTok{, }
  \CommentTok{\# number of bootstraps}
  \CommentTok{\# this is set to a low number for testing}
  \AttributeTok{nboot =} \DecValTok{20}\NormalTok{,}
  \CommentTok{\# name of the fit measure you want to use, make sure it\textquotesingle{}s lavaan code}
  \AttributeTok{invariance\_index =} \StringTok{"cfi"}\NormalTok{,}
  \CommentTok{\# rule for the difference in fit indices}
  \AttributeTok{invariance\_rule =}\NormalTok{ .}\DecValTok{01}\NormalTok{,}
  \CommentTok{\# what order of steps do you want to test? }
  \AttributeTok{group.equal =} \FunctionTok{c}\NormalTok{(}\StringTok{"loadings"}\NormalTok{, }\StringTok{"intercepts"}\NormalTok{, }\StringTok{"residuals"}\NormalTok{)}
\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{kable}\NormalTok{(saved\_boot\_model)}
\end{Highlighting}
\end{Shaded}
...
\hline
intercepts & 0.15 & 0.00 & 0.7953988 & -0.7953988 & 0.2531833 & -0.2531833\\
\hline
loadings & 0.75 & 0.10 & 1.4508940 & -1.4508940 & 0.4618339 & -0.4618339\\
\hline
residuals & 0.10 & 0.05 & 0.1924743 & -0.1924743 & 0.0612665 & -0.0612665\\
\hline
\end{tabular}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{saved\_boot\_partial }\OtherTok{\textless{}{-}} \FunctionTok{bootstrap\_partial}\NormalTok{(}
  \CommentTok{\# the model you want to test }
  \CommentTok{\# use the model before your invariant one}
  \CommentTok{\# in this case it broke down at loadings}
  \AttributeTok{saved\_model =}\NormalTok{ saved\_mgcfa}\SpecialCharTok{$}\NormalTok{model\_configural,}
  \CommentTok{\# the dataframe}
  \AttributeTok{data =}\NormalTok{ HolzingerSwineford1939,}
  \CommentTok{\# the original model syntax}
  \AttributeTok{model =}\NormalTok{ HS.model,}
  \CommentTok{\# the grouping variable column}
  \AttributeTok{group =} \StringTok{"sex"}\NormalTok{,}
  \CommentTok{\# run more but start low for testing}
  \AttributeTok{nboot =} \DecValTok{10}\NormalTok{,}
  \CommentTok{\# what index are you using for invariance?}
  \CommentTok{\# match this to lavaan\textquotesingle{}s name under fitmeasures()}
  \AttributeTok{invariance\_index =} \StringTok{"cfi"}\NormalTok{,}
  \CommentTok{\# what rule are you using?}
  \AttributeTok{invariance\_rule =}\NormalTok{ .}\DecValTok{01}\NormalTok{,}
  \CommentTok{\# what are we comparing against? }
  \AttributeTok{invariance\_compare =} \FunctionTok{unname}\NormalTok{(}\FunctionTok{fitmeasures}\NormalTok{(saved\_mgcfa}\SpecialCharTok{$}\NormalTok{model\_configural, }\StringTok{"cfi"}\NormalTok{)),}
  \CommentTok{\# which step you want to estimate effect size for}
  \AttributeTok{partial\_step =} \FunctionTok{c}\NormalTok{(}\StringTok{"loadings"}\NormalTok{), }
  \CommentTok{\# which parameters you want to hold constrained}
  \AttributeTok{group.equal =} \FunctionTok{c}\NormalTok{(}\StringTok{"loadings"}\NormalTok{)}
\NormalTok{  )}

\FunctionTok{kable}\NormalTok{(}\FunctionTok{head}\NormalTok{(saved\_boot\_partial}\SpecialCharTok{$}\NormalTok{boot\_DF))}

\FunctionTok{kable}\NormalTok{(saved\_boot\_partial}\SpecialCharTok{$}\NormalTok{boot\_summary)}

\FunctionTok{kable}\NormalTok{(saved\_boot\_partial}\SpecialCharTok{$}\NormalTok{boot\_effects)}

\NormalTok{saved\_boot\_partial}\SpecialCharTok{$}\NormalTok{invariance\_plot}

\NormalTok{saved\_boot\_partial}\SpecialCharTok{$}\NormalTok{effect\_invariance\_plot}

\NormalTok{saved\_boot\_partial}\SpecialCharTok{$}\NormalTok{density\_plot}
\end{Highlighting}
\end{Shaded}
